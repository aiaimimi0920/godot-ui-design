...
func _on_h_box_container_resized():
	...
	%EmptyContainer.size = %EmptyContainer.custom_minimum_size
...
func change_clip_points():
	await get_tree().process_frame
	var points = []
	points.append(Vector2(-8,-8))
	if %EmptyContainer.visible == false:
		points.append(Vector2(16,-8))
		points.append(Vector2(16,8))
		points.append(Vector2(16+%EmptyLabelText.size.x,8))
		points.append(Vector2(16+%EmptyLabelText.size.x,-8))
	else:
		pass
	var text_container_size = %TextContainer.size
	points.append(Vector2(text_container_size.x+8,-8))
	points.append(Vector2(text_container_size.x+8,text_container_size.y+8))
	points.append(Vector2(-8,text_container_size.y+8))
	%TextContainerPolygon2D.polygon = points
	%PopulatedLabelText.global_position = %TextContainerPolygon2D.global_position+Vector2(16,-8)
...
func _on_text_container_resized():
	#await get_tree().process_frame
	%TextContainerControl.custom_minimum_size = %TextContainer.custom_minimum_size
	change_clip_points()


func _on_text_container_item_rect_changed():
	await get_tree().process_frame
	%HBoxContainerControl.global_position = %TextContainer.global_position
