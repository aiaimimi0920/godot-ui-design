...
@export var show_mode:ShowMode = ShowMode.FULL_WIDTH:
	set(val):
		...
		if show_mode == ShowMode.FULL_WIDTH:
			%BaseComponent.set("theme_override_constants/margin_top",0)
			%BaseComponent.set("theme_override_constants/margin_bottom",0)
		elif show_mode == ShowMode.MIDDLE_INSET:
			%BaseComponent.set("theme_override_constants/margin_top",8)
			%BaseComponent.set("theme_override_constants/margin_bottom",8)
		elif show_mode == ShowMode.INSET:
			%BaseComponent.set("theme_override_constants/margin_top",16)
			%BaseComponent.set("theme_override_constants/margin_bottom",16)
...
@export var child_init_attr_data = {
		...
		UIDesignConstants.Attr.SHAPE_WIDTH:1,
...
func _on_resized():
	%BaseComponent.custom_minimum_size.y = size.y
	
func _on_base_component_resized():
	self.custom_minimum_size.x = %BaseComponent.size.x
	self.size.x = %BaseComponent.size.x
